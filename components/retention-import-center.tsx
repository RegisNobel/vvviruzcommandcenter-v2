...
import {importErrorCopy, resolveFinalReviewCounts, validateSpotifyCsvFile, type ImportPreviewCounts} from "@/lib/analytics/import-center-ui";
...
export function SpotifyImportWorkflow({artists, releases, canonicalArtistId, initialPreview = null}: {artists: ArtistOption[]; releases: RetentionReleaseOption[]; canonicalArtistId: string | null; initialPreview?: SpotifyImportPreview | null}) {
  ...
  const reviewedSongMappings = validSongRows.flatMap((row) => {
    const decision = mapping[row.originalRowNumber];
    if (decision?.mode === "mapped" && decision.releaseId) return [decision];
    if (decision?.mode === "unmatched" && decision.reason) return [decision];
    return [];
  });
  const warningsAcknowledged = warningCategories.every((warning) => warningAcks[warning.code || "WARNING"]);
  const finalReviewCounts = preview ? resolveFinalReviewCounts(preview.detectedType, preview.counts, {
    releaseConfirmed: Boolean(releaseId),
    mappedSongRows: reviewedSongMappings.filter((item) => item.mode === "mapped").length,
    unmatchedSongRows: reviewedSongMappings.filter((item) => item.mode === "unmatched").length,
    warningAcknowledgementRequired: warningCategories.length > 0,
    warningsAcknowledged
  }) : null;
  const incompleteReviewCopy = preview?.detectedType === "PLAYLISTS_PERIOD"
    ? `Warning review incomplete: acknowledge every required warning category before ${finalReviewCounts?.structurallyValid ?? 0} structurally valid rows can be accepted.`
    : `Mapping review incomplete: ${finalReviewCounts?.reviewed ?? 0} of ${finalReviewCounts?.structurallyValid ?? 0} structurally valid rows have an explicit mapped or unmatched decision.`;
  const pendingAcceptedCopy = preview?.detectedType === "PLAYLISTS_PERIOD" ? "Pending required warning acknowledgements" : "Pending completed mapping review";
