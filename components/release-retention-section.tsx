  ...
  return (
    <section className="scroll-mt-32 space-y-5" id="audience-retention">
      <section className="panel space-y-4 p-4 sm:p-6">
        ...
        {!data.analysis ? <div className={data.selectionState === "DATA_UNAVAILABLE" || data.selectionState === "INVALID_SELECTION" ? "state-panel-danger" : "state-panel-warning"} role="status"><div><p className="font-semibold">{data.selectionState.replaceAll("_", " ")}</p><p className="mt-1 text-sm">{data.selectionMessage}</p><div className="mt-3 flex flex-wrap gap-3"><Link className="action-button-secondary text-xs" href="#campaign-timeline">Manage this release&apos;s campaigns</Link><Link className="action-button-secondary text-xs" href="/admin/retention-lab/imports">Import data</Link><Link className="action-button-secondary text-xs" href="/admin/retention-lab/mappings">Review mappings</Link></div></div></div> : null}
      </section>
