...
async function withKnownActors(model: string, records: SnapshotRecord[] | undefined, fields: string[]) {
  return preserveKnownAdminReferences(prisma, model, records ?? [], fields, provenanceWarnings);
}
...
async function restoreAnalyticsImports(records: SnapshotRecord[] = []) {
  ...
  const actorResolvedRecords = await withKnownActors("AnalyticsImport", records, ["uploadedById", "withdrawnById"]);
...
async function main() {
  ...
  counts.releaseImportAliases = await upsertMany(
    "releaseImportAlias",
    (await withKnownActors("ReleaseImportAlias", snapshot.releaseImportAliases, ["confirmedById", "revokedById"]))
      .map((record) => ({...record, supersededByAliasId: null}))
  );
  ...
  counts.analyticsImportRows = await upsertMany(
    "analyticsImportRow",
    await withKnownActors("AnalyticsImportRow", snapshot.analyticsImportRows, ["confirmedById", "unmatchedById"])
  );
  counts.mappingAuditEvents = await insertManyImmutable(
    "mappingAuditEvent",
    await withKnownActors("MappingAuditEvent", snapshot.mappingAuditEvents, ["actorId"])
  );
  ...
  counts.campaignEvidence = await upsertMany(
    "campaignEvidence",
    (await withKnownActors("CampaignEvidence", snapshot.campaignEvidence, ["createdById"]))
      .map((record) => ({...record, supersededByEvidenceId: null}))
  );
  ...
  counts.campaignActiveIntervals = await upsertMany(
    ...
    (await withKnownActors("CampaignActiveInterval", snapshot.campaignActiveIntervals, ["createdById", "updatedById", "confirmedById", "rejectedById"]))
      .map((record) => ({...record, supersedesIntervalId: null}))
  );
  ...
  counts.campaignTimelineEvents = await upsertMany(
    ...
    (await withKnownActors("CampaignTimelineEvent", snapshot.campaignTimelineEvents, ["createdById", "updatedById"]))
      .map((record) => ({...record, supersedesEventId: null}))
  );
