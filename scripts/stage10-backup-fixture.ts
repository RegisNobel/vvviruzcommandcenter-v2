...
async function fingerprint() {
  ...
  return {
    ...
    actorIds: {
      ...
      campaign: (await prisma.campaignAuditEvent.findUniqueOrThrow({where: {id: "stage10-restore-campaign-audit"}})).actorId
    }
  };
}
...
async function main() {
  ...
  if (mode === "seed") await seed();
    ...
    assert.deepEqual(actual, {...expected, actorIds: {mapping: null, campaign: null}});
